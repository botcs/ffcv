...
Section('data', 'data related stuff').params(
    ...
    train_ids=Param(str, '.pth file to use ids from for training', required=True)
)

@param('data.train_dataset')
@param('data.val_dataset')
@param('data.train_ids')
@param('training.batch_size')
@param('training.num_workers')
def make_dataloaders(train_dataset=None, val_dataset=None, train_ids=None, batch_size=None, num_workers=None):
    paths = {
        'train': train_dataset,
        'test': val_dataset
    ...
    for name in ['train', 'test']:
        ...
        loaders[name] = Loader(paths[name], batch_size=batch_size, num_workers=num_workers,
                               ...
                               indices=(ch.load(train_ids) if name=="train" else None)
                               )
        ...
        for name in ['train', 'test']:
            ...
            for ims, labs in tqdm(loaders[name]):
                with autocast():
                    ...
                    probs.append(out.detach())
                    y_true.append(labs.detach())
                    total_num += ims.shape[0]
            print(f'{name} accuracy: {total_correct / total_num * 100:.1f}%')
            wandb.log({f"{name}_acc": total_correct / total_num * 100})
            probs = ch.cat(probs, dim=0).cpu().numpy()
            y_true = ch.cat(y_true, dim=0).cpu().numpy()
            wandb.log(
                {f"{name}_conf_mat" : wandb.plot.confusion_matrix(
                            y_true=y_true, probs=probs, 
                            class_names=[
...
if __name__ == "__main__":
    ...
    train_ids = os.path.basename(config.get().data.train_ids)[:-4]
    tags = train_ids.split("_") + ["long"]
    wandb.init(
        project="active_cifar10k",
        entity="csbotos",
        config={
            "data": vars(config.get().data),
